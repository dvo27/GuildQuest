...
class QuestScreen(BaseScreen):
    ...
    def refresh_quest_list(self):
        ...
        if view_mode != "all":
            sorted_quests = self.filter_quests_by_view(sorted_quests, view_mode)
        ...
        canvas = tk.Canvas(self.quests_container, bg='#2b2b2b', highlightthickness=0)
        scrollbar = tk.Scrollbar(self.quests_container, orient="vertical", command=canvas.yview)
    ...
    def create_quest_card(self, parent, quest, idx):
        ...
        if quest.required_items:
            info_items.append(f"📦 {len(quest.required_items)} required item(s)")
    ...
    def show_create_quest_dialog(self):
        ...
        # Day
        tk.Label(form_frame, text="Day:", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=2, column=0, sticky='w', pady=5, padx=(20, 0))
        day_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        day_entry.insert(0, str(self.app.world_clock.get_current_time().get_day()))
        day_entry.grid(row=2, column=1, sticky='w', pady=5, padx=(10, 0))
        
        # Hour
        tk.Label(form_frame, text="Hour (0-23):", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=3, column=0, sticky='w', pady=5, padx=(20, 0))
        ...
        # Minute
        tk.Label(form_frame, text="Minute (0-59):", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=4, column=0, sticky='w', pady=5, padx=(20, 0))
        ...
        # End Day
        tk.Label(form_frame, text="Day:", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=7, column=0, sticky='w', pady=5, padx=(20, 0))
        end_day_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        end_day_entry.grid(row=7, column=1, sticky='w', pady=5, padx=(10, 0))
        
        # End Hour
        tk.Label(form_frame, text="Hour (0-23):", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=8, column=0, sticky='w', pady=5, padx=(20, 0))
        end_hour_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        end_hour_entry.grid(row=8, column=1, sticky='w', pady=5, padx=(10, 0))
        
        # End Minute
        tk.Label(form_frame, text="Minute (0-59):", bg='#2b2b2b', fg='white', font=('Courier', 11)).grid(row=9, column=0, sticky='w', pady=5, padx=(20, 0))
        end_minute_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        end_minute_entry.grid(row=9, column=1, sticky='w', pady=5, padx=(10, 0))
        ...
        default_realm = next((name for name, realm in self.app.realms.items() if realm == self.campaign.c_realm), realm_names[0])
        realm_var.set(default_realm)
        
        realm_dropdown = tk.OptionMenu(form_frame, realm_var, *realm_names)
        realm_dropdown.config(bg='#4a4a4a', fg='white', font=('Arial', 11), width=30)
        realm_dropdown.grid(row=10, column=1, sticky='ew', pady=(15, 5), padx=(10, 0))
        ...
        def do_create():
            ...
            if end_day_val or end_hour_val or end_minute_val:
                try:
                    ...
                    if end_day < 0:
                        messagebox.showerror("Error", "End day cannot be negative!")
                        return
                    if end_hour < 0 or end_hour > 23:
                        messagebox.showerror("Error", "End hour must be between 0 and 23!")
                        return
                    if end_minute < 0 or end_minute > 59:
                        messagebox.showerror("Error", "End minute must be between 0 and 59!")
                        return
                    ...
                    messagebox.showerror("Error", "Please enter valid numbers for end time!")
                    return
    ...
    def show_edit_quest_dialog(self, quest, quest_idx):
        ...
        # Day
        tk.Label(form_frame, text="Day:", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=2, column=0, sticky='w', pady=5, padx=(20, 0))
        day_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        day_entry.insert(0, str(quest.time.get_day()))
        day_entry.grid(row=2, column=1, sticky='w', pady=5, padx=(10, 0))
        
        # Hour
        tk.Label(form_frame, text="Hour (0-23):", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=3, column=0, sticky='w', pady=5, padx=(20, 0))
        hour_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        hour_entry.insert(0, str(quest.time.get_hour()))
        hour_entry.grid(row=3, column=1, sticky='w', pady=5, padx=(10, 0))
        
        # Minute
        tk.Label(form_frame, text="Minute (0-59):", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=4, column=0, sticky='w', pady=5, padx=(20, 0))
        minute_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        ...
        if quest.end_time != "N/A":
            ...
            try:
                parts = quest.end_time.replace("Day ", "").replace(",", "").split()
                if len(parts) >= 2:
                    end_day_val = parts[0]
        ...
        # End Day
        tk.Label(form_frame, text="Day:", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=7, column=0, sticky='w', pady=5, padx=(20, 0))
        end_day_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        if end_day_val:
        ...
        # End Hour
        tk.Label(form_frame, text="Hour (0-23):", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=8, column=0, sticky='w', pady=5, padx=(20, 0))
        end_hour_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        if end_hour_val:
        ...
        # End Minute
        tk.Label(form_frame, text="Minute (0-59):", bg='#2b2b2b', fg='white', font=('Arial', 11)).grid(row=9, column=0, sticky='w', pady=5, padx=(20, 0))
        end_minute_entry = tk.Entry(form_frame, width=10, font=('Arial', 11))
        if end_minute_val:
        ...
        realm_names = list(self.app.realms.keys())
        # Set current realm
        current_realm_name = next((name for name, realm in self.app.realms.items() if realm == quest.c_realm), realm_names[0])
